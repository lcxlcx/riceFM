...
try:
    from .ricefm_layers import riceFMLayer, GeneratorLayer
except ImportError:
    import warnings

    warnings.warn("attn is not installed")
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.mod_encoder = ModEncoder(2, d_model)
        ...
        if use_generative_training:
            ...
            self.transformer_encoder = GeneratorLayer(encoder_layers, nlayers)
    ...
    def transformer_generate(
        ...
        mod_type_embs = self.mod_encoder(mod_types)
        ...
        pcpt_total_embs = pcpt_token_embs + pcpt_values + mod_type_embs.unsqueeze(1).repeat(1, pcpt_token_embs.shape[1], 1)
        ...
        if gen_genes is not None:
            ...
            gen_total_embs = gen_token_embs + gen_flags + mod_type_embs.unsqueeze(1).repeat(1, gen_token_embs.shape[1], 1)
